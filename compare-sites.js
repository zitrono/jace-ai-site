...
async function compareSites() {
  ...
  try {
    ...
    for (const [name, site] of Object.entries(results)) {
      ...
      try {
        ...
        const tests = {
          ...
          heroSubtitle: await page
            .$eval('h1 + p', (el) => el.textContent.trim())
            .catch(() => 'NOT FOUND'),
          ...
          features: await page
            .$$eval('#features h3, [id*="feature"] h3', (els) => els.length)
            .catch(() => 0),
          testimonials: await page
            .$$eval('[class*="testimonial"] blockquote', (els) => els.length)
            .catch(() => 0),
          faqItems: await page.$$eval('[class*="faq"] button', (els) => els.length).catch(() => 0),
          logo: await page
            .$eval('nav a span', (el) => el.textContent.trim())
            .catch(() => 'NOT FOUND'),
          casaBadge: await page
            .$eval('.badge-certification', (el) => el.textContent.trim())
            .catch(() => 'NOT FOUND'),
          userCount: await page
            .$eval('[data-test="user-count"]', (el) => el.textContent.trim())
            .catch(() => 'NOT FOUND'),
        };
        ...
        tests.mobileMenuButton = (await page.$(
    ...
    console.log('\n🔑 KEY OBSERVATIONS:');
    console.log(
      `- Ralph (migrated) site: ${ralphResults.error ? '❌ ERROR: ' + ralphResults.error : '✅ Working'}`
    );
    console.log(
      `- Jace (official) site: ${jaceResults.error ? '❌ ERROR: ' + jaceResults.error : '✅ Working'}`
    );
    console.log(
      `- Ralph has ${ralphResults.features || 0} features, Jace has ${jaceResults.features || 0}`
    );
    console.log(
      `- Ralph has ${ralphResults.testimonials || 0} testimonials, Jace has ${jaceResults.testimonials || 0}`
    );
  } finally {
    await browser.close();
  }
