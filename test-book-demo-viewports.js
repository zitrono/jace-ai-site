...
async function testBookDemoPage() {
  ...
  for (const viewport of viewports) {
    ...
    if (iframeInfo) {
      ...
      console.log(
        `   ✓ Iframe size: ${Math.round(iframeInfo.iframeWidth)}x${Math.round(iframeInfo.iframeHeight)}`
      );
      console.log(
        `   ✓ Container size: ${Math.round(iframeInfo.containerWidth)}x${Math.round(iframeInfo.containerHeight)}`
      );
