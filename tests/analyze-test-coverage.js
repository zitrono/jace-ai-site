...
function analyzeTestCoverage() {
  ...
  const validationMethods = allMethods.filter(m => m.startsWith('validate') && typeof mainPOM[m] === 'function');
  const interactiveMethods = allMethods.filter(m => m.startsWith('test') && typeof mainPOM[m] === 'function');
  ...
  console.log(`Maximum possible properties: ${propPOM.cssProperties.length * Object.keys(propPOM.coreSelectors).length}`);
  
  // 4. Coverage Summary
  console.log('\n\n✅ COVERAGE SUMMARY:');
  console.log(`Main POM selector coverage: ${(testedSelectors/totalSelectors * 100).toFixed(1)}% (${testedSelectors}/${totalSelectors})`);
