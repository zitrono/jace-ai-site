  ...
  try {
    ...
    const testimonialsData = await page.evaluate(() => {
      ...
      const data = {
        ...
        sectionSubtitle: Array.from(testimonialsSection.querySelectorAll('p'))
          .find((p) => p.textContent?.includes(searchText))
          ?.textContent?.trim(),
        testimonials: [],
      };

      // Find testimonial cards
      const cards = testimonialsSection.querySelectorAll(
        'div[class*="bg-white"], div[class*="card"], blockquote'
      );
