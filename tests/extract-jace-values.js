  ...
  try {
    ...
    const extractedData = await page.evaluate(() => {
      ...
      data.content = {
        ...
        faqQuestions: Array.from(document.querySelectorAll('button[aria-expanded]')).map((b) =>
          b.textContent?.trim()
        ),
      ...
      data.structure = {
        ...
        hasTestimonials: Array.from(document.querySelectorAll('h2')).some((h) =>
      ...
      data.elements = {
        ...
        userCountText: Array.from(document.querySelectorAll('span'))
          .find((s) => s.textContent?.includes('Join'))
          ?.textContent?.trim(),
      };
    ...
    // Save extracted data
    const outputPath =
      '/Users/zitrono/dev/web/ralph-web/jace-ai-site/tests/jace-extracted-values.json';
