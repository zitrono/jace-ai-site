...
export class JaceAISitePOM {
  ...
  mobileRequirements = {
    ...
    mobileMenu: {
      // CRITICAL: Jace reference implementation details (measured 2025-06-24)
      // Jace has transparent overlay and fixed-width panel
      overlay: {
        backgroundColor: 'rgba(0, 0, 0, 0)', // Jace uses FULLY transparent overlay
        width: '100vw',
        height: '100vh',
        zIndex: 50,
      },
      panel: {
        width: 320, // Fixed width in pixels, NOT percentage
        backgroundColor: 'rgb(40, 40, 40)', // Matches page background exactly
        position: 'fixed',
        rightAligned: true, // Uses right: 0, not left: 0
        padding: 24, // pixels
        ctaLayout: 'flex-row',
        ctaButtonWidth: 272, // Fixed pixel width for Jace
        loginLink: {
          padding: '10px 24px',
          fontSize: '18px',
        },
    ...
    const baseSelectors = {
      ...
      hero: {
        ...
        ctaButtonHero: {
          ...
          selector:
            'main button[class*="btn-primary"][class*="btn-lg"], main button[class*="bg-surface-highlight"]',
        },
        ctaButtonHeader: {
          jaceSelector: 'header button[class*="bg-surface-highlight"]',
          selector:
            'header button[class*="btn-primary"], header button[class*="bg-surface-highlight"]',
        },
      ...
      navigation: {
        ...
        logo: {
          ...
          selector:
            'header a[class*="inline-flex"][class*="items-center"], header a[class*="-m-1.5"]',
        },
      ...
      faq: {
        ...
        items: {
          jaceSelector:
            'button[class*="group"][class*="flex"][class*="w-full"][class*="cursor-pointer"]',
          selector:
            'button[class*="w-full"][class*="text-left"][class*="flex"], button[class*="group"][class*="w-full"]',
        },
        questions: {
          jaceSelector: 'dt button[aria-expanded], button[class*="group"][class*="cursor-pointer"]',
          selector:
            'button[class*="w-full"][class*="flex"][class*="items-center"][class*="justify-between"], dt button',
        },
  ...
  expectedStyles = {
    ...
    testimonials: {
      ...
      card: {
        ...
        padding: '24px',
        boxShadow:
          'rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, oklab(0.21 -0.00316127 -0.0338527 / 0.05) 0px 0px 0px 1px, rgba(0, 0, 0, 0.1) 0px 10px 15px -3px, rgba(0, 0, 0, 0.1) 0px 4px 6px -4px',
      },
      quote: {
        fontSize: '14px',
        lineHeight: '24px',
  ...
  structuralRequirements = {
    ...
    requiredElements: [
      ...
      },
      {
        ...
        selector:
          'img[alt*="Google"], img[alt*="Meta"], img[alt*="Amazon"], img[class*="company"], img[alt*="logo"]',
  ...
  functionalRequirements = {
    ...
    mobileMenuVisualParity: {
      ...
      header: {
        showLogo: true, // Logo added for visual parity with Jace
        marginBottom: '24px', // Spacing after header
        justifyContent: 'space-between', // Logo left, close button right
      },
    },
    ...
    faqAccordion: {
      ...
      triggers:
        'button[class*="w-full"][class*="flex"], button[class*="group"][class*="cursor-pointer"], dt button, .faq-button',
  ...
  async measureReference(selector, viewports = [320, 360, 375]) {
    const measurements = {};

    for (const viewport of viewports) {
      await this.page.setViewport({ width: viewport, height: 568 });
      await new Promise((resolve) => setTimeout(resolve, 300)); // Allow layout to settle

      measurements[viewport] = await this.page.evaluate((sel) => {
  ...
  // Comparative validation method (helper method, not a test)
  async compareElementToReference(config) {
    const errors = [];

    // Measure reference
    await this.page.goto(this.jaceUrl);
    await new Promise((resolve) => setTimeout(resolve, 500));

    // Open mobile menu in reference if needed
    if (config.needsMobileMenu) {
    ...
    const referenceData = await this.measureReference(config.referenceSelector);
    ...
    const implementationData = await this.measureReference(config.implementationSelector);

    // Compare across all viewports
    for (const viewport in referenceData) {
      const ref = referenceData[viewport];
      const impl = implementationData[viewport];

      if (!ref) {
        errors.push(`${viewport}px: Reference element not found`);
        continue;
      }

      if (!impl) {
        errors.push(`${viewport}px: Implementation element not found`);
        continue;
      }

      // Width comparison with tolerance
      const widthDiff = Math.abs(ref.widthPercent - impl.widthPercent);
      if (widthDiff > (config.tolerance || 5)) {
        errors.push(
          `${viewport}px: Width ${impl.widthPercent.toFixed(1)}% vs reference ${ref.widthPercent.toFixed(1)}% (diff: ${widthDiff.toFixed(1)}%)`
        );
      }

      // Layout pattern comparison if specified
      if (config.validateLayout && ref.styles.flexDirection !== impl.styles.flexDirection) {
        errors.push(
          `${viewport}px: Layout direction ${impl.styles.flexDirection} vs reference ${ref.styles.flexDirection}`
        );
      }
    }

    return { errors, referenceData, implementationData };
  }

  // Compare mobile menu visual properties
  async compareMobileMenuVisuals() {
    const errors = [];

    // Measure Jace visuals
    await this.page.goto(this.jaceUrl);
    await new Promise((resolve) => setTimeout(resolve, 500));

    // Open Jace mobile menu
    await this.page.evaluate(() => {
      const menuButtons = document.querySelectorAll('button');
      for (let btn of menuButtons) {
        const svg = btn.querySelector('svg');
    ...
    const jaceLayout = await this.page.evaluate(() => {
      ...
      // Check for logo
      const textElements = menuPanel.querySelectorAll('a, span, div, h1, h2, h3');
      for (let elem of textElements) {
        if (
          elem.textContent.toLowerCase().includes('jace') &&
          elem.getBoundingClientRect().y < 150
        ) {
          layout.hasLogo = true;
          break;
        }
      ...
      // Check buttons - look for any clickable elements with button-like text
      const buttons = [];
      const allElements = menuPanel.querySelectorAll('*');
      allElements.forEach((elem) => {
        ...
        if (
          ...
          rect.width > 50 &&
          rect.height > 30 &&
          (text.includes('Log') || text.includes('Get Started') || text.includes('Free')) &&
      ...
      layout.buttonCount = buttons.length;

      // Determine button layout
      if (buttons.length >= 2) {
        const btn1 = buttons[0].rect;
        const btn2 = buttons[1].rect;
        layout.buttonLayout = Math.abs(btn1.y - btn2.y) < 10 ? 'horizontal' : 'vertical';
      }
      ...
      links.forEach((link) => {
        ...
        if (
          text &&
          !text.toLowerCase().includes('jace') &&
          !text.includes('Log') &&
          !text.includes('Get Started')
        ) {
          layout.navigationLinksCount++;
        }
      });
    ...
    // Measure Ralph layout
    await this.page.goto(this.url);
    await new Promise((resolve) => setTimeout(resolve, 500));

    // Open Ralph mobile menu
    await this.page.evaluate(() => {
      const menuButton = document.querySelector('#mobile-menu-button');
      if (menuButton) menuButton.click();
    });
    ...
    const ralphLayout = await this.page.evaluate(() => {
      ...
      for (let elem of textElements) {
        if (
          elem.textContent.toLowerCase().includes('ralph') &&
          elem.getBoundingClientRect().y < 150
        ) {
          layout.hasLogo = true;
          break;
      ...
      // Check buttons
      const buttons = [];
      const buttonElements = menuPanel.querySelectorAll('button, .btn');
      ...
      links.forEach((link) => {
        ...
        if (
          text &&
          !text.toLowerCase().includes('ralph') &&
          !text.includes('Log') &&
          !text.includes('Book')
        ) {
          layout.navigationLinksCount++;
    ...
    // Compare layouts
    if (!jaceLayout || !ralphLayout) {
      errors.push('Could not analyze mobile menu layouts');
      return { errors };
    }

    console.log('📐 Layout Comparison:');
    console.log('  Jace:', jaceLayout);
    console.log('  Ralph:', ralphLayout);

    if (jaceLayout.hasLogo !== ralphLayout.hasLogo) {
      errors.push(
        `Logo presence mismatch - Jace: ${jaceLayout.hasLogo}, Ralph: ${ralphLayout.hasLogo}`
      );
    }

    if (jaceLayout.buttonLayout !== ralphLayout.buttonLayout) {
      // Skip validation if Jace has unknown layout (no buttons detected)
      if (jaceLayout.buttonLayout !== 'unknown') {
        errors.push(
          `Button layout mismatch - Jace: ${jaceLayout.buttonLayout}, Ralph: ${ralphLayout.buttonLayout}`
        );
      } else if (ralphLayout.buttonLayout !== 'vertical' && ralphLayout.buttonCount > 0) {
        // If Jace has no buttons but Ralph does, ensure Ralph uses vertical layout
        errors.push(`Ralph button layout should be vertical when buttons are present`);
      }
    }

    return { errors, jaceLayout, ralphLayout };
  }
  ...
  async navigate(url = null, target = null) {
    ...
    if (target) {
      ...
      this.target =
        targetUrl.includes('jace.ai') || targetUrl.includes('localhost:8081') ? 'jace' : 'ralph';
    }
    ...
    if (headerButtonExists) {
      ...
      if (buttonStyles) {
        ...
        if (!acceptablePaddings.includes(buttonStyles.padding)) {
          errors.push(
            `Header button padding mismatch at ${viewport.width}px: ${buttonStyles.padding} (expected: ${acceptablePaddings.join(' or ')})`
          );
        }
        ...
        if (
          buttonStyles.height &&
          buttonStyles.height !== expectedHeight &&
          buttonStyles.height !== 'auto'
        ) {
          errors.push(
            `Header button height mismatch at ${viewport.width}px: ${buttonStyles.height} (expected: ${expectedHeight})`
          );
        }
      }
    } else {
      errors.push(
        `Header button not found with selector: ${typeof headerButtonSelector === 'object' ? headerButtonSelector.selector : headerButtonSelector}`
      );
    }
  ...
  async validateCheckmarkColors() {
    ...
    if (checkmarkExists) {
      const checkmarkStyles = await this.getElementStyles(
        this.selectors.checkmarks.pricingCheckmarks
      );
      if (checkmarkStyles) {
        ...
        const colorMatches =
          checkmarkStyles.color === expectedColor ||
          checkmarkStyles.color === 'rgb(16, 185, 129)' ||
          checkmarkStyles.color.includes('oklch');

        if (!colorMatches) {
          errors.push(
            `Checkmark color mismatch: ${checkmarkStyles.color} (expected: ${expectedColor})`
          );
        }
      }
    } else {
  ...
  async validateFAQInteractivity() {
    ...
    try {
      ...
      if (firstButton.cursor !== expected.cursor) {
        errors.push(
          `FAQ button cursor mismatch: ${firstButton.cursor} (expected: ${expected.cursor})`
        );
      }

      if (firstButton.pointerEvents !== expected.pointerEvents) {
        errors.push(
          `FAQ button pointerEvents mismatch: ${firstButton.pointerEvents} (expected: ${expected.pointerEvents})`
        );
      }
  ...
  async validateSpecialBackgrounds() {
    ...
    const gradientSection = await this.elementExists(
      this.selectors.specialBackgrounds.gradientSection
    );
  ...
  async validateCookieConsent() {
    ...
    try {
      ...
      if (cookieBanner) {
        ...
        const acceptButtonExists = await this.elementExists(
          this.selectors.cookieConsent.acceptButton
        );
        ...
        const rejectButtonExists = await this.elementExists(
          this.selectors.cookieConsent.rejectButton
        );
        ...
        const settingsButtonExists = await this.elementExists(
          this.selectors.cookieConsent.settingsButton
        );
      ...
      const cookiePolicyExists = await this.elementExists(
        this.selectors.cookieConsent.cookiePolicyLink
      );
      ...
      if (!isJaceReference) {
        ...
        if (subtitleElement) {
          ...
          if (fontSize < minSize || fontSize > maxSize) {
            const percentDiff = Math.abs(((fontSize - expectedSize) / expectedSize) * 100).toFixed(
              1
            );
            errors.push(
              `Font size outside 5% tolerance: ${fontSize}px vs ${expectedSize}px (${percentDiff}% difference)`
            );
          }
      ...
      if (titleElement) {
        ...
        if (titleFontSize < minTitleSize || titleFontSize > maxTitleSize) {
          const percentDiff = Math.abs(
            ((titleFontSize - expectedTitleSize) / expectedTitleSize) * 100
          ).toFixed(1);
          errors.push(
            `Hero title size outside 5% tolerance: ${titleFontSize}px vs ${expectedTitleSize}px (${percentDiff}% difference)`
          );
        }
      }
    } catch (error) {
  ...
  async validateNavigation() {
    ...
    if (navLinks.length === 0) {
      ...
      if (!navStyles.fontSize.match(expectedDesktop.fontSize)) {
        errors.push(
          `Desktop nav font size: ${navStyles.fontSize} (expected: ${expectedDesktop.fontSize})`
        );
      }
      if (!navStyles.fontWeight.match(expectedDesktop.fontWeight)) {
        errors.push(
          `Desktop nav font weight: ${navStyles.fontWeight} (expected: ${expectedDesktop.fontWeight})`
        );
      }
    }
  ...
  async validateCompanyLogos() {
    ...
    const companyExists =
      (await this.elementExists(this.selectors.companies.section)) ||
  ...
  async validateTestimonials() {
    ...
    const testimonialsExist = await this.page.evaluate(() => {
      ...
      const testimonialCards = document.querySelectorAll(
        '.card-white, [class*="testimonial"], figure'
      );
      const testimonialsSection = Array.from(document.querySelectorAll('h2, h3')).find(
        (h) =>
  ...
  async validateFAQ() {
    ...
    const faqExists = await this.page.evaluate(() => {
      ...
      const faqButtons = document.querySelectorAll(
        'button[aria-expanded], [class*="faq"], [class*="accordion"]'
      );
      const headingsWithFAQ = Array.from(document.querySelectorAll('h1, h2, h3')).find(
        (h) =>
  ...
  async validateAll() {
    const results = {
      ...
      headerTransparency: await this.validateHeaderTransparency(),
  ...
  async testHoverStates() {
    ...
    try {
      ...
      for (const element of hoverElements.slice(0, 3)) {
  ...
  async testFAQAccordion() {
    ...
    if (this.target === 'jace') {
      ...
      for (const button of allButtons) {
        ...
        const hasToggleFAQ = await button.evaluate((btn) =>
      ...
      if (buttonToClick) {
        ...
        if (this.target === 'jace') {
          ...
          if (!hasClickHandler) {
            ...
            await new Promise((resolve) => setTimeout(resolve, 300));
            const finalExpanded = await buttonToClick.evaluate((el) =>
          ...
          const isNowExpanded = await buttonToClick.evaluate((el) =>
            el.getAttribute('aria-expanded')
          );
        ...
        const hasAriaExpanded = await firstButton.evaluate((el) =>
  ...
  async testCookieConsentFunctionality() {
    ...
    try {
      ...
      if (cookieBanner) {
        ...
        console.log(
          '   ℹ️ No cookie banner to test (implementation may not include cookie consent)'
        );
      }
    ...
    return async (...args) => {
      ...
      for (const error of errors) {
        ...
        for (const [overrideId, override] of Object.entries(this.overrides.specs)) {
          if (
            error.includes(override.selector) ||
  ...
  async validateMobileLayout(viewportName = 'narrow') {
    ...
    if (headerHeight > this.mobileRequirements.header.maxHeight) {
      errors.push(
        `Mobile header too tall: ${headerHeight}px (max: ${this.mobileRequirements.header.maxHeight}px)`
      );
    }
      ...
      (minGap, target) => {
        ...
        const hamburger =
          ...
          Array.from(document.querySelectorAll('header button')).find(
            (btn) => btn.textContent.includes('Open main menu') || btn.querySelector('svg')
  ...
  async validateHeaderTransparency() {
    const errors = [];

    // Check header background and transparency
    const headerCheck = await this.page.evaluate(() => {
      ...
      // Check if header overlaps with content
      const heroContent = document.querySelector('h1') || document.querySelector('main > *');
      ...
      return {
        ...
        hasBackdrop: styles.backdropFilter !== 'none',
        overlapsContent: heroRect ? headerRect.bottom > heroRect.top : false,
      };
    });

    if (headerCheck) {
      // Check if header is properly floating with backdrop
      if (!headerCheck.isFloating) {
        errors.push('Header should be floating (position: fixed, absolute, or sticky)');
      }

      if (!headerCheck.hasBackdrop && headerCheck.backgroundColor.includes('rgba')) {
        errors.push('CRITICAL: Header lacks backdrop-filter but uses transparent background');
      }

      // Header should be semi-transparent for floating effect
      const bgColor = headerCheck.backgroundColor;
      if (bgColor.includes('rgb(') && !bgColor.includes('rgba(')) {
        errors.push(
          `Header background is fully opaque: ${bgColor} (expected: semi-transparent for floating effect)`
        );
      }
    }
  ...
  async validateMobileMenuPanel() {
    ...
    // Open mobile menu
    const menuButton =
      (await this.page.$('button[aria-label*="menu"]')) ||
    ...
    if (menuButton) {
      ...
      if (!overlayStyles) {
        ...
        if (overlayStyles.backgroundColor !== expected.backgroundColor) {
          errors.push(
            `Overlay background: ${overlayStyles.backgroundColor} (expected: ${expected.backgroundColor})`
          );
        }
      }
      ...
      const panelStyles = await this.page.evaluate(() => {
        const panel =
          document.querySelector('[data-mobile-menu-panel]') ||
      ...
      if (panelStyles) {
        ...
        if (panelStyles.backgroundColor !== expectedPanel.backgroundColor) {
          errors.push(
            `Mobile menu panel background: ${panelStyles.backgroundColor} (expected: ${expectedPanel.backgroundColor})`
          );
        }

        // Critical transparency check - hero text should not be visible behind solid menu
        if (
          panelStyles.heroTextVisibleBehindMenu &&
          panelStyles.heroRect &&
          panelStyles.panelRect
        ) {
          const overlap =
            panelStyles.heroRect.left < panelStyles.panelRect.right &&
          ...
          if (overlap) {
            errors.push(
              'CRITICAL: Mobile menu is transparent - hero text visible behind menu panel'
            );
          }
        }
      }
      ...
      const ctaLayout = await this.page.evaluate(() => {
        const buttons =
          document.querySelectorAll('.mobile-menu button.btn-primary') ||
      ...
      if (ctaLayout && ctaLayout.flexDirection !== 'row') {
        errors.push(
          `Mobile menu CTAs should be side-by-side (flex-row), found: ${ctaLayout.flexDirection}`
        );
      }
      ...
      const mobileNavStyles = await this.page.evaluate(() => {
        const mobileLinks = document.querySelectorAll(
          '#mobile-menu nav a, #mobile-menu-overlay nav a, [data-mobile-menu-panel] nav a'
        );
      ...
      if (mobileNavStyles) {
        ...
        if (!mobileNavStyles.fontSize.match(expectedMobile.fontSize)) {
          errors.push(
            `CRITICAL: Mobile nav font size: ${mobileNavStyles.fontSize} (expected: ${expectedMobile.fontSize})`
          );
        }
        ...
        if (fontWeightNum !== 500) {
          errors.push(
            `CRITICAL: Mobile nav font weight: ${mobileNavStyles.fontWeight} (expected: 500 - fonts appear too thin/bold)`
          );
        }
        ...
        if (mobileNavStyles.fontFamily && !mobileNavStyles.fontFamily.includes('Geist')) {
          errors.push(
            `Mobile nav font family: ${mobileNavStyles.fontFamily} (expected: contains 'Geist')`
          );
        }
      }

      // Reference-based width validation - measure Jace and compare to Ralph
      if (this.target === 'ralph') {
        const mobileMenuConfig = {
          referenceSelector:
            'div[style*="fixed"], .mobile-menu, #mobile-menu div:last-child, [class*="mobile"]',
          implementationSelector: '[data-mobile-menu-panel]',
          needsMobileMenu: true,
          tolerance: 8, // Allow 8% difference for responsive scaling
          validateLayout: true,
        };

        const comparison = await this.compareElementToReference(mobileMenuConfig);

        if (comparison.errors.length > 0) {
          comparison.errors.forEach((error) => {
            errors.push(`MOBILE MENU WIDTH: ${error}`);
          });
        }

        // Log measurements for debugging
        if (comparison.referenceData && comparison.implementationData) {
          console.log('📏 Mobile Menu Measurements:');
          for (const viewport in comparison.referenceData) {
            const ref = comparison.referenceData[viewport];
            const impl = comparison.implementationData[viewport];
            if (ref && impl) {
              console.log(
                `  ${viewport}px: Jace ${ref.widthPercent.toFixed(1)}% vs Ralph ${impl.widthPercent.toFixed(1)}%`
              );
            }
          }
        }

        // Additional layout composition validation
        const layoutComparison = await this.compareMobileMenuLayout();
        if (layoutComparison.errors.length > 0) {
          layoutComparison.errors.forEach((error) => {
            errors.push(`LAYOUT: ${error}`);
          });
        }
      }

      // Check for logo presence in mobile menu (Jace has "jace" logo, Ralph should have "ralph")
      const logoCheck = await this.page.evaluate(() => {
        const mobileMenu = document.querySelector('#mobile-menu');
        if (!mobileMenu || mobileMenu.classList.contains('hidden')) return null;

        const logo =
          mobileMenu.querySelector('a[href*="/ralph-web/"]') ||
          mobileMenu.querySelector('.logo') ||
          mobileMenu.querySelector('[class*="logo"]');
      ...
      if (logoCheck && this.target === 'ralph') {
        if (!logoCheck.hasLogo) {
          errors.push(
            'CRITICAL: Mobile menu missing logo - Jace reference has "jace" logo, Ralph should have "ralph" logo'
          );
        }
      }

      // Check background color parity (Jace has lighter grayish background)
      const backgroundColorCheck = await this.page.evaluate(() => {
        ...
        // Convert rgb to values for comparison
        const rgbMatch = bg.match(/rgb\((\d+),\s*(\d+),\s*(\d+)\)/);
        if (!rgbMatch) return { backgroundColor: bg, isTooDark: false };
        ...
        return {
          ...
          isTooDark: brightness < 60, // neutral-600 has brightness ~65
          rgbValues: { r, g, b },
        };
      });

      if (backgroundColorCheck && this.target === 'ralph') {
        if (backgroundColorCheck.isTooDark) {
          errors.push(
            `CRITICAL: Mobile menu background too dark (brightness: ${backgroundColorCheck.brightness}) - should match Jace's lighter grayish tone`
          );
        }
      }

      // Check button visual parity across multiple resolutions
      const buttonParityCheck = async () => {
        const resolutions = [
        ...
        for (const res of resolutions) {
          await this.page.setViewport(res);
          await new Promise((resolve) => setTimeout(resolve, 100));

          const buttonCheck = await this.page.evaluate((resInfo) => {
            const mobileMenu = document.querySelector('#mobile-menu');
            if (!mobileMenu || mobileMenu.classList.contains('hidden')) return null;

            const ctaButton = mobileMenu.querySelector(
              'button[class*="btn-primary"], .btn-primary'
            );
            const loginButton = mobileMenu.querySelector(
              'button[class*="btn-secondary"], .btn-secondary'
            );
            ...
            return {
              resolution: resInfo.name,
              viewportWidth: window.innerWidth,
              ctaButton: getButtonInfo(ctaButton),
              loginButton: getButtonInfo(loginButton),
              buttonsContainer: mobileMenu.querySelector('.flex.flex-col')
                ? 'flex-col'
                : mobileMenu.querySelector('.flex-col')
                  ? 'flex-col'
                  : 'other',
            };
          }, res);
      ...
      if (this.target === 'ralph') {
        const buttonResults = await buttonParityCheck();

        for (const result of buttonResults) {
          // Check if CTA button is properly sized and visible
          if (result.ctaButton) {
            if (!result.ctaButton.visible) {
              errors.push(`CRITICAL: CTA button not visible on ${result.resolution}`);
            }
            // Use device-appropriate width thresholds for mobile-first design
            const minWidth = result.viewportWidth <= 375 ? 75 : 80; // 75px for mobile devices, 80px for larger screens
            if (result.ctaButton.width < minWidth) {
              errors.push(
                `CRITICAL: CTA button too narrow on ${result.resolution}: ${result.ctaButton.width}px (expected: >${minWidth}px)`
              );
            }
          } else {
            errors.push(`CRITICAL: CTA button not found on ${result.resolution}`);
          }

          // Check button layout
          if (result.buttonsContainer !== 'flex-col') {
            errors.push(`Button layout issue on ${result.resolution}: expected flex-col layout`);
          }
        }
      ...
      const visualParityCheck = await this.page.evaluate(() => {
        const panel =
          ...
          document.querySelector('#mobile-menu > div:last-child');
        const backdrop =
          document.querySelector('#mobile-menu > div:first-child') ||
          document.querySelector('.fixed.inset-0.bg-black\\/50');
        const navLinks = document.querySelectorAll('.mobile-nav-link');
        const buttonSection =
          document.querySelector('#mobile-menu .border-t') ||
      ...
      if (visualParityCheck.panel) {
        if (visualParityCheck.panel.padding !== expectedVisual.panel.padding) {
          errors.push(
            `Panel padding: ${visualParityCheck.panel.padding} (expected: ${expectedVisual.panel.padding})`
          );
        }
        if (visualParityCheck.panel.borderRadius !== expectedVisual.panel.borderRadius) {
          errors.push(
            `Panel border radius: ${visualParityCheck.panel.borderRadius} (expected: ${expectedVisual.panel.borderRadius})`
          );
        }
      }

      if (
        visualParityCheck.backdrop &&
        visualParityCheck.backdrop.opacity !== expectedVisual.backdrop.opacity
      ) {
        errors.push(
          `Backdrop opacity: ${visualParityCheck.backdrop.opacity} (expected: ${expectedVisual.backdrop.opacity})`
        );
      }
      ...
      if (visualParityCheck.navLinks) {
        if (visualParityCheck.navLinks.padding !== expectedVisual.navigationLinks.padding) {
          errors.push(
            `Nav link padding: ${visualParityCheck.navLinks.padding} (expected: ${expectedVisual.navigationLinks.padding})`
          );
        }
        if (
          visualParityCheck.navLinks.borderRadius !== expectedVisual.navigationLinks.borderRadius
        ) {
          errors.push(
            `Nav link border radius: ${visualParityCheck.navLinks.borderRadius} (expected: ${expectedVisual.navigationLinks.borderRadius})`
          );
        }
      }

      if (
        visualParityCheck.header &&
        visualParityCheck.header.hasLogo !== expectedVisual.header.showLogo
      ) {
        errors.push(
          `Mobile menu logo: ${visualParityCheck.header.hasLogo ? 'shown' : 'hidden'} (expected: ${expectedVisual.header.showLogo ? 'shown' : 'hidden'})`
        );
      }
      ...
      if (backgroundCheck) {
        ...
        if (!backgroundCheck.panelBackground.match(solidRgbPattern)) {
          errors.push(
            `Panel background not solid: ${backgroundCheck.panelBackground} (expected: solid rgb())`
          );
        }

        // Check for any transparency in the chain
        const transparentElements = backgroundCheck.transparencyChain.filter(
          (el) =>
            el.opacity !== '1' ||
        ...
        if (transparentElements.length > 0) {
          errors.push(
            `Transparency found in panel hierarchy: ${JSON.stringify(transparentElements[0])}`
          );
        }
      }
      ...
      const fontWeightCheck = await this.page.evaluate(() => {
        ...
        return {
          ...
          buttonWeights: Array.from(buttons)
  ...
  async validateIOSHorizontalScroll() {
    ...
    try {
      ...
      if (scrollStyles.bodyOverflowX !== 'hidden' && scrollStyles.htmlOverflowX !== 'hidden') {
        errors.push(
          `Horizontal overflow not prevented. body overflow-x: ${scrollStyles.bodyOverflowX}, html overflow-x: ${scrollStyles.htmlOverflowX}`
        );
      }
      ...
      if (
        !validTouchActions.includes(scrollStyles.bodyTouchAction) &&
        !validTouchActions.includes(scrollStyles.htmlTouchAction)
      ) {
        errors.push(
          `Touch action not properly set. body: ${scrollStyles.bodyTouchAction}, html: ${scrollStyles.htmlTouchAction}`
        );
      }
      ...
      if (scrollStyles.viewportMeta) {
        ...
        if (!hasWidthDevice || !hasInitialScale) {
          errors.push(
            `Viewport meta tag missing required attributes: ${scrollStyles.viewportMeta}`
          );
        }
      } else {
      ...
      if (swipeResult.problematicElements.length > 0) {
        errors.push(
          `Elements extending beyond viewport: ${JSON.stringify(swipeResult.problematicElements[0])}`
        );
      }
    } catch (error) {
  ...
  async testIOSHorizontalSwipe() {
    ...
    try {
      ...
      if (afterScroll.x !== initialScroll.x) {
        errors.push(
          `Page scrolled horizontally during swipe. Initial: ${initialScroll.x}, After: ${afterScroll.x}`
        );
      }
    } catch (error) {
  ...
  async validateStructuralIntegrity() {
    ...
    for (const relationship of this.structuralRequirements.layoutRelationships) {
      if (relationship.parent && relationship.children) {
        ...
        if (parentEl) {
          const childCount = await this.page.evaluate(
            (parent, childrenSelectors) => {
          ...
          if (relationship.minChildren && childCount < relationship.minChildren) {
            errors.push(
              `${relationship.description}: found ${childCount} children, expected minimum ${relationship.minChildren}`
            );
          }
        }
      }
      ...
      if (relationship.selector && relationship.nextSibling) {
        const hasValidSibling = await this.page.evaluate(
          (sel, sibling) => {
            const element = document.querySelector(sel);
  ...
  async validateLayoutConstraints() {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (sectionBox) {
        ...
        if (sectionInfo.paddingTop < constraints.top) {
          errors.push(
            `Section ${i} padding-top too small: ${sectionInfo.paddingTop}px (min: ${constraints.top}px)`
          );
        }
        if (sectionInfo.paddingBottom < constraints.bottom) {
          errors.push(
            `Section ${i} padding-bottom too small: ${sectionInfo.paddingBottom}px (min: ${constraints.bottom}px)`
          );
        }
      }
    }
    ...
    const heroElement = await this.page.$(
      '.hero, main > section:first-child, main > div:first-child'
    );
    if (heroElement) {
      ...
      if (heroBox) {
        if (heroBox.height < this.layoutValidation.heroLayout.minHeight) {
          errors.push(
            `Hero section too short: ${heroBox.height}px (min: ${this.layoutValidation.heroLayout.minHeight}px)`
          );
        }
        if (heroBox.height > this.layoutValidation.heroLayout.maxHeight) {
          errors.push(
            `Hero section too tall: ${heroBox.height}px (max: ${this.layoutValidation.heroLayout.maxHeight}px)`
          );
        }
      }
    }
  ...
  async validateMobileLayoutConstraintsInternal(errors = []) {
    ...
    if (header) {
      ...
      if (
        headerBox &&
        headerBox.height > this.layoutValidation.mobileConstraints.header.maxHeight
      ) {
        localErrors.push(
          `Mobile header too tall: ${headerBox.height}px (max: ${this.layoutValidation.mobileConstraints.header.maxHeight}px)`
        );
      }
    }
    ...
    const touchTargets = await this.page.$$(
      this.layoutValidation.mobileConstraints.touchTargets.selectors.join(', ')
    );
    for (const target of touchTargets) {
      ...
      if (
        isPrimaryTarget &&
        (box.width < this.layoutValidation.mobileConstraints.touchTargets.minSize ||
        ...
        localErrors.push(
          `Touch target too small: ${tagName}${className ? `.${className.split(' ')[0]}` : ''} (${box.width}x${box.height}px, min: ${this.layoutValidation.mobileConstraints.touchTargets.minSize}px)`
        );
      }
    }

    // Element spacing validation
    const logoCta = await this.checkElementSpacing(
      '.logo, [class*="logo"]',
  ...
  async validateEnhancedFunctionalBehavior() {
    ...
    for (const [selector, requirements] of Object.entries(
      this.functionalRequirements.interactiveStates
    )) {
      const elements = await this.page.$$(selector);
    ...
    try {
      ...
      if (newState === 'true') {
        const overlay = await this.page.$(
          '.overlay, [class*="overlay"], .backdrop, [class*="backdrop"]'
        );
        if (overlay) {
          ...
          if (
            !overlayStyles.backgroundColor.includes('rgba(0, 0, 0') &&
            !overlayStyles.backgroundColor.includes('rgb(0, 0, 0')
          ) {
            errors.push(
              `Menu overlay background: ${overlayStyles.backgroundColor} (expected: ${expectedBg})`
            );
          }
        ...
        if (panel) {
          ...
          if (panelStyles.backgroundColor !== expectedPanelBg) {
            localErrors.push(
              `Menu panel background: ${panelStyles.backgroundColor} (expected: ${expectedPanelBg})`
            );
          }
        }
      }
  ...
  async validateEnhancedFAQInternal(errors = []) {
    ...
    try {
      ...
      const initialAriaExpanded = await faqButton.evaluate((el) =>
        el.getAttribute('aria-expanded')
      );
  ...
  // Enhanced multi-layer detection for accurate measurements
  async findMenuLayers(siteType = 'jace') {
    return await this.page.evaluate((type) => {
      ...
      // Find all potential menu elements
      ...
      for (let elem of allElements) {
        ...
        if (rect.height < 100 || styles.display === 'none') continue;

        if (styles.position === 'fixed' || (elem.id && elem.id.includes('menu'))) {
          candidates.push({
            element: elem,
            id: elem.id,
            ...
            height: rect.height,
            x: rect.x,
            hasContent: elem.querySelector('a, button') !== null,
            hasNavLinks: elem.querySelector('a[href*="#"]') !== null,
            backgroundColor: styles.backgroundColor,
            zIndex: styles.zIndex,
            position: styles.position,
            left: styles.left,
      ...
      // Identify layers by characteristics
      // Sort by z-index and content to find correct layers
      candidates.sort((a, b) => {
      ...
      candidates.forEach((candidate) => {
        // Full viewport transparent/semi-transparent overlay
        if (
          candidate.width === window.innerWidth &&
          candidate.height >= window.innerHeight * 0.9 &&
          !candidate.hasContent &&
          (candidate.backgroundColor.includes('rgba') ||
            candidate.backgroundColor === 'transparent')
        ) {
          layers.overlay = candidate;
        }
        // Menu panel with content
        else if (candidate.hasNavLinks && candidate.hasContent) {
          // For Jace: fixed 320px width panel
          // For Ralph: could be full width or percentage
          layers.panel = candidate;
        }
        // Backdrop (if separate from overlay)
        else if (
          candidate.width === window.innerWidth &&
          !candidate.hasContent &&
          candidate.backgroundColor.includes('rgb(')
        ) {
          layers.backdrop = candidate;
        }
      });
      ...
      return {
        ...
        allCandidates: candidates, // For debugging
      };
    }, siteType);
  }

  // Enhanced mobile menu validation with multi-layer awareness
  async validateMobileMenuStructure() {
    const results = [];
    const viewports = [320, 375, 414];

    for (const viewport of viewports) {
      await this.page.setViewport({ width: viewport, height: 667 });
      await new Promise((resolve) => setTimeout(resolve, 300));

      // Open menu
      await this.page.click(this.isRefactor ? '#mobile-menu-button' : 'div.d-block.d-lg-none');
      await new Promise((resolve) => setTimeout(resolve, 500));

      // Find all layers
      const menuData = await this.findMenuLayers(this.isRefactor ? 'ralph' : 'jace');

      if (!menuData.layers.panel) {
        results.push({
          viewport,
          passed: false,
          message: 'Could not find menu panel',
          details: menuData,
        });
        continue;
      }

      const panel = menuData.layers.panel;
      const expectedWidth = this.mobileRequirements.mobileMenu.panel.width;
      const tolerance = 10; // pixels

      const passed = Math.abs(panel.width - expectedWidth) <= tolerance;

      results.push({
        viewport,
        passed,
        expected: `${expectedWidth}px (±${tolerance}px)`,
        actual: `${panel.width}px`,
        backgroundColor: panel.backgroundColor,
        position: `x: ${panel.x}, right: ${panel.right}`,
        message: passed
          ? 'Menu width matches reference'
          : `Menu width mismatch: expected ${expectedWidth}px, got ${panel.width}px`,
      });
    }
  ...
  // Compare mobile menu layout between implementations
  async compareMobileMenuLayout() {
    const errors = [];

    try {
      // This is a simplified version - the full implementation would measure both menus
      const menuPanel = await this.page.$('[data-mobile-menu-panel]');
      if (!menuPanel) {
        errors.push('Mobile menu panel not found');
        return { errors };
      }

      const menuData = await this.page.evaluate((el) => {
        const rect = el.getBoundingClientRect();
      ...
      const expectedWidth = this.mobileRequirements.mobileMenu.panel.width;
      const viewport = await this.page.viewport();
      const actualExpectedWidth = viewport.width <= 320 ? viewport.width : expectedWidth;

      if (Math.abs(menuData.width - actualExpectedWidth) > 10) {
        errors.push(`Width mismatch: got ${menuData.width}px, expected ${actualExpectedWidth}px`);
      }

      if (menuData.backgroundColor !== 'rgb(40, 40, 40)') {
        errors.push(`Background color mismatch: got ${menuData.backgroundColor}`);
      }
    } catch (error) {
      errors.push(`Layout comparison failed: ${error.message}`);
    }
