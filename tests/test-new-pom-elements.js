// Test new POM elements with original site values
import puppeteer from 'puppeteer';
import { UniversalJaceAISitePOM } from './universal-jace-pom.js';

async function testNewPOMElements() {
  console.log('🧪 Testing new POM elements against original site...');
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      const pom = new UniversalJaceAISitePOM(page, 'original');
      await pom.navigate();
    ...
    if (overallSuccess === '100.0') {
      console.log('🎉 Perfect! All new POM elements pass with original values.');
    } else {
