...
async function runTest({ target = 'ralph' }) {
  ...
  try {
    ...
    const passRate =
      typeof propertyResults.grandTotal.passRate === 'number'
        ? propertyResults.grandTotal.passRate
        : ((propertyResults.grandTotal.tested - propertyResults.grandTotal.errors) /
            propertyResults.grandTotal.tested) *
          100;
...
async function runComprehensivePropertyTests(pom, page, target) {
  ...
  for (const [name, selectorDef] of Object.entries(coreElements)) {
    console.log(
      `📋 Testing ${name} (${typeof selectorDef === 'string' ? selectorDef : selectorDef.selector})`
    );
    ...
    try {
      ...
      if (name === 'heroTitle' && pom.expectedStyles.typography?.heroTitle) {
        ...
        if (expected.fontSize && !properties.fontSize.match(expected.fontSize)) {
          propertyErrors.push(
            `${name}.fontSize: Expected ${expected.fontSize}, got "${properties.fontSize}"`
          );
        }
      }
      ...
      if (name === 'casaBadge' && pom.expectedStyles.components?.casaBadge) {
        ...
        if (expected.borderRadius && !properties.borderRadius.match(expected.borderRadius)) {
          propertyErrors.push(
            `${name}.borderRadius: Expected ${expected.borderRadius}, got "${properties.borderRadius}"`
          );
        }
      }
      ...
      if (
        ...
        !properties.borderWidth.match(/^(\d+px\s*)+$/)
      ) {
      ...
      if (name === 'ctaButton' && pom.expectedStyles.typography?.button?.primary) {
        ...
        if (expected.fontWeight && !properties.fontWeight.match(expected.fontWeight)) {
          propertyErrors.push(
            `${name}.fontWeight: Expected ${expected.fontWeight}, got "${properties.fontWeight}"`
          );
        }
      }
    } catch (error) {
  ...
  results.grandTotal.passRate =
    results.grandTotal.tested > 0
